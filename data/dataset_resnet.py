...
class CLIPGraspingDataset(torch.utils.data.Dataset):
    ...
    def load_entries(self):
        ...
        with gzip.open("shapenet-np.json.gz", 'r') as f:
            self.img_raw = json.load(f)
    ...
    def get_img_feats_resnet(self, key):
        feats = []
        for i in range(self.total_views):
            img_feats_path = f"raw_np/{key}-{i}.npz.npy"
            feat = np.load(img_feats_path)
            # feat = np.array(self.img_feats[f'{key}-{i}'])
            feats.append(feat)
        return np.array(feats)
    ...
    def __getitem__(self, idx):
        ...
        # read raw image
        img1_n_raw_feats = torch.from_numpy(self.get_img_feats_resnet(key1))[start_idx:]
        img2_n_raw_feats = torch.from_numpy(self.get_img_feats_resnet(key2))[start_idx:]
        # label
        ans = entry_idx

        return (
            (img1_n_raw_feats, img2_n_raw_feats),
